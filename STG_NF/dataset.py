...
def gen_dataset(
    ...
    **dataset_args,
    ...
    segs_data_np, segs_score_np, segs_meta = [], [], []
    global_data, person_keys = [], {}
...
def keypoints17_to_coco18(kps):
    ...
    """
    kp_np = np.array(kps)
    neck_kp = 0.5 * (kp_np[..., 5, :] + kp_np[..., 6, :])
    kp_np = np.concatenate([kp_np, neck_kp[..., None, :]], axis=-2)
    # Reorder according to the target order.
    opp_order = np.array([0, 17, 6, 8, 10, 5, 7, 9, 12, 14, 16, 11, 13, 15, 2, 1, 4, 3])
    return kp_np[..., opp_order, :]
