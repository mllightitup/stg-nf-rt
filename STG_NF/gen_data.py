...
def convert_data_format(data, split="None"):
    ...
    for item in data:
        ...
        if person_idx_str not in data_new:
            data_new[person_idx_str] = {
                frame_idx_str: {"keypoints": keypoints, "scores": scores}
            }
        else:
            data_new[person_idx_str][frame_idx_str] = {
                "keypoints": keypoints,
                "scores": scores,
            }
...
def create_command(alphapose_dir, video_filename, out_dir, is_video=False):
    command_args = {
        "cfg": os.path.join(
            alphapose_dir, "configs/coco/resnet/256x192_res152_lr1e-3_1x-duc.yaml"
        ),
        "checkpoint": os.path.join(
            alphapose_dir, "pretrained_models/fast_421_res152_256x192.pth"
        ),
        "outdir": out_dir,
    }
...
def main():
    ...
    ap.add_argument(
        "--dir", dest="dir", type=str, required=True, help="video\images dir"
    )
    ap.add_argument(
        "--outdir", dest="outdir", type=str, required=True, help="video\images outdir"
    )
    ap.add_argument(
        "--alphapose_dir",
        dest="alphapose_dir",
    ...
    for path, subdirs, files in os.walk(root):
        for name in files:
            ...
            if run_pose:
                ...
                alphapose_tracked_results_filename = (
                    video_basename + "_alphapose_tracked_person.json"
                )
                ...
                command = create_command(
                    args.alphapose_dir, video_filename, args.outdir, is_video=args.video
                )
                # Run command
                print("\n$", command)
                os.system(command)
                ...
                read_convert_write(
                    alphapose_orig_results_filename, alphapose_tracked_results_filename
                )
