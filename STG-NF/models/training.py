...
class Trainer:
    def __init__(
        self, args, model, train_loader, test_loader, optimizer_f=None, scheduler_f=None
    ):
        self.model = model
        self.args = args
    ...
    def save_checkpoint(self, epoch, is_best=False, filename=None):
        ...
        if is_best:
            shutil.copy(
                path_join, os.path.join(self.args.ckpt_dir, "checkpoint_best.pth.tar")
            )
    ...
    def train(self, log_writer=None, clip=100):
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            for itern, data_arr in enumerate(pbar):
                try:
                    data = [
                        data.to(self.args.device, non_blocking=True)
    ...
    def test(self):
        ...
        for itern, data_arr in enumerate(pbar):
            ...
            with torch.no_grad():
                z, nll = self.model(
                    samp.float(), label=torch.ones(data[0].shape[0]), score=score
                )
            if self.args.model_confidence:
                nll = nll * score
